---
title: Quick Start
description: Get started with Hanggent in just a few minutes!
icon: rocket
---

This guide will walk you through building your first multi-agent workforce using Hanggent.
...
- **Windows:** `C:\Users\[YourUsername]\hanggent\[YourEmailPrefix]\task_[TaskID]`
- **Mac:** `/Users/[YourUsername]/hanggent/[YourEmailPrefix]/task_[TaskID]`
...
Hanggent comes with four ready-to-work agents. Each is equipped with a specific set of tools and shines at specific tasks—click to explore:
...
Type your task in the top Message Box. Be as descriptive as you like. For example, ask Hanggent to conduct an UK healthcare market research . You can attach files (like docs, data, images) by clicking the **paperclip icon** in the Message Box.Then, hit **Send**.
...
Once you're happy with the plan, hit **Start Task.** Hanggent will automatically assign each subtask to the best agent for the job based on the tools they have.
...
Your privacy is important to us! Because Hanggent agents operate on your desktop, they may need permissions to take screenshots or access local files to complete tasks. You can enable and manage all these permissions here.
...
Hanggent can run in two modes. Your choice here affects how you are billed and what models are available.
...
- **Deployment Policy (Ops):** In staging and production, default-provider auto-seeding is disabled (`AUTO_SEED_DEFAULT_PROVIDERS='false'`) and enforced in the deploy workflows (`.github/workflows/deploy-staging.yml`, `.github/workflows/deploy-vultr-k8s.yml`).
