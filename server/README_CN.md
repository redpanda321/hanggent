...
# 停止/启动 API 容器（保留数据库）
docker stop hanggent_api
docker start hanggent_api
...
# 查看日志
docker logs -f hanggent_api | cat
docker logs -f hanggent_postgres | cat
```
...
# 3) 本地启动（需提供数据库连接串）
 ...
 export database_url=postgresql://postgres:123456@localhost:5432/hanggent
 uv run uvicorn main:api --reload --port 3001 --host 0.0.0.0

 # 方式 B：在 server/.env 中写入（示例）
 # database_url=postgresql://postgres:123456@localhost:5432/hanggent
