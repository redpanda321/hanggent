...
class ChatHistory(AbstractModel, DefaultTimes, table=True):
    ...
    project_name: str = Field(default="", sa_column=Column(String(512)))
    ...
    def update_fields(self, update_dict: dict):
        """Override to sanitize fields before saving."""
        # Strip <think> tags from project_name and summary
        if "project_name" in update_dict and isinstance(update_dict["project_name"], str):
            update_dict["project_name"] = self._strip_think_tags(update_dict["project_name"])
            if len(update_dict["project_name"]) > 512:
                update_dict["project_name"] = update_dict["project_name"][:509] + "..."
        if "summary" in update_dict and isinstance(update_dict["summary"], str):
            update_dict["summary"] = self._strip_think_tags(update_dict["summary"])
        # Convert status int to ChatStatus enum for ChoiceType compatibility
        if "status" in update_dict and isinstance(update_dict["status"], int):
            try:
                update_dict["status"] = ChatStatus(update_dict["status"])
            except ValueError:
                pass  # Let SQLAlchemy raise a proper validation error
        super().update_fields(update_dict)
