...
def get_user_credits(auth: Auth = Depends(auth_must), session: Session = Depends(session)):
    """Get user's current credit balance (dollar amount) and daily free token allowance."""
    user = auth.user
    user.refresh_credits_on_active(session)

    # Dollar credit balance (from subscriptions / top-ups minus consumed costs)
    dollar_credits = float(user.credits or 0)

    # Daily free token allowance (tracked separately in UserCreditsRecord)
    daily_credits: UserCreditsRecord | None = UserCreditsRecord.get_daily_balance(user.id)
    current_daily_tokens = 0
    if daily_credits:
        remaining = daily_credits.amount - daily_credits.balance
        current_daily_tokens = remaining if remaining > 0 else 0

    logger.debug(
        "Credits retrieved",
        extra={"user_id": user.id, "dollar_credits": dollar_credits, "daily_tokens": current_daily_tokens},
    )
    # Return dollar credits and daily free tokens as separate fields (different units)
    return {"credits": dollar_credits, "daily_credits": current_daily_tokens}


@router.get("/user/invite_code", name="get user invite code")
def get_invite_code(auth: Auth = Depends(auth_must), session: Session = Depends(session)):
    """Get or generate the current user's unique invite/referral code."""
    user: User = auth.user

    if not user.invite_code:
        # Lazily generate a short URL-safe code on first request
        for _attempt in range(5):
            candidate = secrets.token_urlsafe(6)  # 8 chars
            ...
            logger.error("Failed to generate unique invite code after retries", extra={"user_id": user.id})
            return {"invite_code": None}

        logger.info("Invite code generated", extra={"user_id": user.id, "invite_code": user.invite_code})
