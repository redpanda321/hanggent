...
import { getViteBaseURL } from '@/lib/index';
 ...
 * Uses VITE_BASE_URL if set, otherwise falls back to window.location.origin.
 */
function getOAuthBaseURL(): string {
  ...
  // In production without explicit VITE_BASE_URL, use the current origin
  if (typeof window !== 'undefined') {
    return window.location.origin;
  }
  return 'https://www.hangent.com'; // Fallback for SSR or non-browser contexts
}

export class OAuth {
  public client_name: string = 'Hanggent';
  public client_uri: string = 'https://hangent.com/';
  ...
  async startOauth(mcpName: string) {
    ...
    this.redirect_uris = [
      `${getOAuthBaseURL()}/api/oauth/${this.provider}/callback`,
    ];
  ...
  async refreshToken(provider: string, email: string) {
    ...
    if (getPlatformService().isElectron) {
      await getPlatformService().env.write(email, {
