...
export function useIntegrationManagement(items: IntegrationItem[]) {
  ...
  const saveEnvAndConfig = useCallback(
    async (provider: string, envVarKey: string, value: string) => {
      ...
      if (getPlatformService().isElectron) {
        await getPlatformService().env.write(email, { key: envVarKey, value });
      }
    },
    [configs, email]
  );
  ...
  useEffect(() => {
    ...
    platform.ipc.on('oauth-authorized', handler);
    return () => {
      platform.ipc.off('oauth-authorized', handler);
    };
  }, [processOauth]);
  ...
  useEffect(() => {
    ...
    const platform = getPlatformService();
    platform.ipc.on('oauth-callback-url', handler);
    return () => {
      platform.ipc.off('oauth-callback-url', handler);
    };
  }, []);
      ...
      for (const config of toDelete) {
        try {
          ...
          if (
            ...
            await getPlatformService().env.remove(email, item.env_vars[0]);
          }
        } catch (_e) {
          // Ignore error
        }
