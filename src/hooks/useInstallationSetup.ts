 ...
 * In web/mobile environments, this hook is a no-op since electronAPI is not available.
 */
export const useInstallationSetup = () => {
  ...
  // In web mode, skip the entire installation flow and go straight to 'done'
  useEffect(() => {
    if (!electronAvailable && initState !== 'done') {
      console.log('[useInstallationSetup] Web mode detected, skipping installation flow');
      setInitState('done');
    }
  }, [electronAvailable, initState, setInitState]);
  ...
  const startBackendPolling = useCallback(() => {
    ...
    const checkBackendStatus = async () => {
      try {
        ...
        const backendPort = await platform.backend.getBackendPort();
        if (backendPort && backendPort > 0) {
          ...
          const isHealthy = await platform.backend.checkHealth();
          if (isHealthy) {
            console.log(
  ...
  }, [electronAvailable, setSuccess, setInitState, setNeedsBackendRestart]);
  ...
  useEffect(() => {
    if (!electronAvailable || hasCheckedOnMount.current) {
      return;
    }
    ...
    const checkToolInstalled = async () => {
      try {
        const platform = getPlatformService();
        const result = await platform.install.checkToolInstalled();
    ...
    const checkBackendStatus = async (_toolResult?: any) => {
      try {
        ...
        const installationStatus =
          await platform.install.getInstallationStatus();
  ...
  useEffect(() => {
    ...
    const platform = getPlatformService();
    const cleanupInstallEvents = platform.install.onInstallEvents({
      onStart: handleInstallStart,
      onLog: handleInstallLog,
      onComplete: handleInstallComplete,
      onBackendReady: handleBackendReady,
    });

    return () => {
      cleanupInstallEvents();
    };
  }, [
    startInstallation,
    addLog,
