...
export default function SettingGeneral() {
  ...
  useEffect(() => {
    const platform = getPlatformService().window.getPlatform();
  ...
  useEffect(() => {
    ...
    const loadProxyConfig = async () => {
      try {
        const result = await getPlatformService().env.readGlobalEnv('HTTP_PROXY');
        if (result?.value) {
  ...
  const handleSaveProxy = async () => {
    ...
    try {
      if (trimmed) {
        const result = await getPlatformService().env.write(authStore.email, {
          key: 'HTTP_PROXY',
          value: trimmed,
        });
        if (!result?.success) throw new Error('envWrite returned no success');
      } else {
        const result = await getPlatformService().env.remove(
          authStore.email,
          'HTTP_PROXY'
        );
      ...
      <div className="mb-8 flex flex-col gap-6">
        ...
        <div className="item-center flex flex-row justify-between rounded-2xl bg-surface-secondary px-6 py-4">
          ...
          <div className="flex items-center gap-sm">
            <Button
              onClick={() => {
                window.location.href = `https://www.hangent.com/dashboard?email=${authStore.email}`;
              }}
              variant="primary"
              size="sm"
            >
          ...
          <div className="flex w-full flex-row items-center gap-md">
            {themeList.map((item: any) => (
              ...
              >
                <img
                  src={item.img}
                  className={`group-hover:border-bg-fill-info-primary aspect-[183/91.67] w-full rounded-lg border border-solid border-transparent transition-all ${item.value == appearance
                ...
                />
                <div
                  className={`text-sm text-text-primary group-hover:underline ${item.value == appearance ? 'underline' : ''
                    }`}
                >
                  {t(item.label)}
                </div>
        ...
        <Input
          ...
          trailingButton={
            <Button
              ...
              onClick={
                proxyNeedsRestart
                  ? () => getPlatformService().window.restartApp()
                  : handleSaveProxy
              }
