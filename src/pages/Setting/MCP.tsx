  ...
  const handleConfigSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (getPlatformService().isElectron) {
        ...
        getPlatformService().mcp.update(mcpData.mcp_name, payload);
      }
      ...
      if (addType === 'local') {
        ...
        if (getPlatformService().isElectron) {
          ...
          for (const [key, value] of Object.entries(mcpServers)) {
            await getPlatformService().mcp.install(key, value);
          }
        }
      }
      setShowAdd(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (getPlatformService().isElectron) {
        ...
        await getPlatformService().mcp.remove(deleteTarget.mcp_key);
      }
