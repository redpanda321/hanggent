...
import {
  ...
  proxyFetchGet,
  proxyFetchPost,
  proxyFetchPostSilent,
  proxyFetchPut,
  uploadFile,
  waitForBackendReady,
} from '@/api/http';
import { showCreditsToast } from '@/components/Toast/creditsToast';
import { showSpendingAlertToast } from '@/components/Toast/spendingAlertToast';
...
/**
 * Infer the cloud provider name from a model_type string.
 * E.g. "gpt-4o" → "openai", "qwen-plus" → "qwen", "claude-sonnet-4-…" → "anthropic".
 */
function inferCloudProvider(modelType: string): string {
  const t = modelType.toLowerCase();
  if (t.includes('/')) return 'openrouter';
  if (t.includes('gpt') || t.includes('o1-') || t.includes('o3-') || t.includes('o4-')) return 'openai';
  ...
  if (t.includes('moonshot') || t.includes('kimi')) return 'kimi';
  if (t.includes('glm') || t.includes('chatglm')) return 'glm';
  if (t.includes('yi-')) return 'lingyiwanwu';
  return 'openai-compatible-model';
}

/**
 * Resolve the current model configuration based on the user's active
 * settings (cloud / custom / local). Returns model params that can be
 * included in improve / follow-up API calls so the backend uses the
 * model the user actually selected — not the one from the initial task.
 */
export async function resolveCurrentModelConfig(): Promise<{
  ...
  if (modelType === 'custom' || modelType === 'local') {
    const { getModelStore } = await import('@/store/modelStore');
    const activeProvider = getModelStore().getActiveProvider(modelType);
    if (!activeProvider) return null;
    return {
      api_key: activeProvider.api_key,
      model_type: activeProvider.model_type,
      model_platform: activeProvider.model_platform,
      api_url: activeProvider.api_url,
      extra_params: activeProvider.extra_params ?? {},
    };
  }

  if (modelType === 'cloud') {
    const cloudProvider = inferCloudProvider(cloud_model_type);

    // Pre-check available providers to find the right one for this model
    let primaryProvider = cloudProvider;
    try {
      ...
      if (availableProviders.length > 0) {
        const providerNames = availableProviders.map((p: any) => p.provider_name);
        if (!providerNames.includes(cloudProvider)) {
          const matching = availableProviders.find(
            (p: any) => p.model_type === cloud_model_type
          );
          if (matching) {
            primaryProvider = matching.provider_name;
          }
        }
      }
    } catch {
      // ignore — proceed with inferred provider
    }

    // Try to resolve a real API key from the admin-configured providers
    try {
      const resolveRes = await proxyFetchPostSilent(
        `/api/cloud/resolve-key?provider_name=${encodeURIComponent(primaryProvider)}&model_type=${encodeURIComponent(cloud_model_type)}`,
        {}
      );
      if (resolveRes?.api_key) {
        return {
          api_key: resolveRes.api_key,
          model_type: resolveRes.model_type || cloud_model_type,
          model_platform: resolveRes.provider_name || primaryProvider,
          api_url: resolveRes.api_url || '',
          extra_params: {},
        };
      }
    } catch {
    ...
    try {
      const resolveRes = await proxyFetchPostSilent(
        `/api/cloud/resolve-key?provider_name=openrouter&model_type=${encodeURIComponent(cloud_model_type)}`,
        {}
      );
      if (resolveRes?.api_key) {
        return {
          api_key: resolveRes.api_key,
          model_type: resolveRes.model_type || cloud_model_type,
          model_platform: resolveRes.provider_name || 'openrouter',
          api_url: resolveRes.api_url || '',
          extra_params: {},
        };
      }
    ...
    // Last resort: user key via LiteLLM proxy
    try {
      ...
      if (res?.api_url) {
        return {
          ...
          model_platform: primaryProvider,
          api_url: res.api_url,
          extra_params: {},
        };
      }
...
const chatStore = (initial?: Partial<ChatStore>) =>
  createStore<ChatStore>()((set, get) => ({
    ...
    create(id?: string, type?: any) {
      ...
      // Pre-populate with opencode_agent so the workspace defaults to OpenCode
      const defaultAgentId = `opencode_default_${taskId}`;
      const defaultAgent: ChatBox.Agent = {
      ...
      set((state) => ({
        ...
        tasks: {
          ...
          [taskId]: {
            ...
            progressValue: 0,
            isPending: false,
            activeWorkSpace: defaultAgentId,
            hasMessages: false,
            activeAgent: '',
    ...
    stopTask(taskId: string) {
      ...
      // Check spending alert after task completes (fire-and-forget)
      proxyFetchGet('/usage/stats')
        .then((stats: any) => {
          if (stats?.alert_threshold_reached && !stats?.limit_reached) {
            showSpendingAlertToast();
          }
        })
        .catch(() => {
    ...
    startTask: async (
      ...
      if (language === 'system') {
        systemLanguage = await getPlatformService().system.getSystemLanguage();
      }
      if (type === 'replay') {
      ...
      const base_Url = import.meta.env.DEV
        ...
        : (typeof window !== 'undefined' && (window as any).__ENV?.VITE_BASE_URL) || import.meta.env.VITE_BASE_URL;
      const api =
        type == 'share'
      ...
      if (modelType === 'custom' || modelType === 'local') {
        // Read from localStorage via modelStore (no server call)
        const { getModelStore } = await import('@/store/modelStore');
        const activeProvider = getModelStore().getActiveProvider(modelType);
        ...
        const resolveCloud = async (provider: string) =>
          proxyFetchPostSilent(
            `/api/cloud/resolve-key?provider_name=${encodeURIComponent(provider)}&model_type=${encodeURIComponent(cloud_model_type)}`,
            {}
          );

        const applyResolved = (resolveRes: any, providerFallback: string) => {
          apiModel = {
            api_key: resolveRes.api_key,
            model_type: resolveRes.model_type || cloud_model_type,
            model_platform: resolveRes.provider_name || providerFallback,
            api_url: resolveRes.api_url || '',
            extra_params: {},
          };
        };

        const fallbackToUserKey = async () => {
          const res = await proxyFetchGet('/api/user/key');
          if (!res?.api_url) {
            throw new Error(
              'Cloud mode is not configured on this environment (missing LiteLLM proxy URL).'
            );
          }
          ...
          apiModel = {
            api_key: res.value,
            model_type: cloud_model_type,
            model_platform: cloudProvider,
            api_url: res.api_url,
            extra_params: {},
          };
        };

        // Resolve cloud key: get real provider API key from admin config.
        // Pre-check available providers to avoid noisy 404s in the fallback chain.
        let primaryProvider = cloudProvider;
        try {
          const res = await proxyFetchGet('/api/cloud/available-providers');
          const availableProviders =
          ...
          if (availableProviders.length > 0) {
            const providerNames = availableProviders.map((p: any) => p.provider_name);
            if (!providerNames.includes(cloudProvider)) {
              // Current model's provider isn't configured — try to find one that has this model_type
              const matching = availableProviders.find((p: any) =>
                p.model_type === cloud_model_type
              );
              if (matching) {
                console.log(`[startTask] Provider '${cloudProvider}' not configured, using '${matching.provider_name}' instead`);
                primaryProvider = matching.provider_name;
              } else {
                console.warn(`[startTask] No configured provider supports model '${cloud_model_type}', falling through to fallback chain`);
              }
            }
          }
        } catch {
          // available-providers check failed — proceed with normal fallback chain
          console.warn('[startTask] Could not fetch available providers, using fallback chain');
        }

        // If the direct provider isn't configured, try OpenRouter as a fallback.
        try {
          const resolveRes = await resolveCloud(primaryProvider);
          if (resolveRes?.api_key) {
            applyResolved(resolveRes, primaryProvider);
          } else {
            throw new Error('Cloud key resolution returned no api_key');
          }
        } catch {
          try {
            const resolveRes = await resolveCloud('openrouter');
            if (resolveRes?.api_key) {
              applyResolved(resolveRes, 'openrouter');
            } else {
              throw new Error('Cloud key resolution returned no api_key');
            }
          } catch {
            // Last resort: only valid when LiteLLM proxy is configured.
            await fallbackToUserKey();
          }
        }
      }
      ...
      if (getPlatformService().isElectron) {
        mcpLocal = await getPlatformService().mcp.list();
      }
      ...
      let envPath = '';
      try {
        envPath = await getPlatformService().env.getEnvPath(email);
      } catch (error) {
      ...
      const browser_port = await getPlatformService().system.getBrowserPort();
      ...
      fetchEventSource(api, {
        ...
        async onmessage(event: any) {
          ...
          try {
            agentMessages = JSON.parse(eventData);
          } catch (error) {
            console.warn('Failed to parse SSE message, ignoring frame:', error);
            console.warn('Raw event.data:', eventData);
            return;
          }
          ...
          if (agentMessages.step === AgentStep.TO_SUB_TASKS) {
            ...
            if (!type && historyId) {
              // Strip <think>...</think> blocks from AI-generated text
              ...
              const obj = {
                project_name: stripThinkTags(rawName),
                summary: stripThinkTags(rawSummary),
                status: 1,
                ...
                if (agent_name == 'browser_agent') {
                  snapshots.forEach((item: any) => {
                    const imgurl = !item.image_path.includes('/public')
                      ...
                      : (import.meta.env.DEV
                          ? import.meta.env.VITE_PROXY_URL
                          : (typeof window !== 'undefined' && (window as any).__ENV?.VITE_BASE_URL) || import.meta.env.VITE_BASE_URL) + item.image_path;
                ...
                if (
                  ...
                  item.activeWebviewIds.map((webview, index) => {
                    if (webview.processTaskId === task_id) {
                      getPlatformService().webview.destroy(webview.id);
          ...
          if (agentMessages.step === AgentStep.ERROR) {
            try {
              ...
              const errorMessage =
                ...
                agentMessages.data?.detail?.message ||
                agentMessages.data?.error?.message ||
                (typeof agentMessages.data === 'string'
                  ? agentMessages.data
                  : null) ||
                (typeof agentMessages.data === 'object'
                  ? normalizeToolkitMessage(agentMessages.data)
                  : null) ||
          ...
          if (agentMessages.step === AgentStep.END) {
            ...
            // Async file upload
            let res = await getPlatformService().file.getFileList(
              email,
              currentTaskId,
            ...
            if (
              ...
              const uploadResults = await Promise.allSettled(
                res
                  ...
                  .map(async (file: any) => {
                    try {
                      // Read file content using platform service
                      const result = await getPlatformService().file.readFile(
                        file.path
                      );
        ...
        onerror(err) {
          ...
          // Always stop retrying for POST-based SSE connections.
          // Auto-retrying creates duplicate step_solve sessions on the backend
          // with corrupted state. The user should see the error and retry manually.
          console.error(
            '[fetchEventSource] SSE connection error, stopping:',
            err
          );
    ...
    clearTasks: () => {
      ...
      getPlatformService().backend.restartBackend()
        .then((res) => {
