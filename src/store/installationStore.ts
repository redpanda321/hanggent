  ...
  subscribeWithSelector((set, get) => ({
    ...
    retryBackend: async () => {
      try {
        ...
        // Call restart-backend via platform service
        const { getPlatformService } = await import('@/service/platform');
        const result = await getPlatformService().backend.restartBackend();
    ...
    performInstallation: async () => {
      ...
      try {
        startInstallation();
        const { getPlatformService } = await import('@/service/platform');
        const result = await getPlatformService().install.checkAndInstallDepsOnUpdate();
