...
const Layout = () => {
  ...
  useEffect(() => {
    const handleBeforeClose = () => {
      ...
      if (['running', 'pause'].includes(currentStatus)) {
        ...
        getPlatformService().window.closeWindow(true);
      }
    };
    ...
    return () => {
      platform.ipc.removeAllListeners('before-close');
    };
  ...
  // In web mode, skip the install screen entirely — no Electron dependencies to install
  const webMode = isWeb();
  const actualShouldShowInstallScreen = webMode
    ? false
    : shouldShowInstallScreen ||
      initState !== 'done' ||
