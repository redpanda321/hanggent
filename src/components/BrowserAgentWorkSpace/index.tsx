...
export default function Home() {
  ...
  const handleTakeControl = (id: string) => {
    ...
    setTimeout(() => {
      ...
      getPlatformService().webview.showWebview(id);
    }, 400);
  };
  ...
  useEffect(() => {
    ...
    platform.ipc.on('url-updated', (_event: any, newUrl: any) => {
      setUrl(newUrl);
    });

    // optional: clear listener when uninstall
    return () => {
      platform.ipc.removeAllListeners('url-updated');
    };
  }, []);
        ...
        <div className="rounded-full border border-solid border-border-primary bg-transparent p-1">
          <Button
            onClick={() => {
              ...
              getPlatformService().webview.hideAllWebview();
            }}
            size="sm"
            variant="success"
