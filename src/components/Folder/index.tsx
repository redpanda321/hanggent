...
export default function Folder({ data: _data }: { data?: Agent }) {
  ...
  const selectedFileChange = (file: FileInfo, isShowSourceCode?: boolean) => {
    if (file.type === 'zip') {
      ...
      getPlatformService().file.revealInFolder(file.path);
      return;
    }
    ...
    getPlatformService().file
      .openFile(file.type, file.path, isShowSourceCode)
      .then((res: any) => {
  ...
  useEffect(() => {
    ...
    const setFileList = async () => {
      let res = null;
      res = await getPlatformService().file.getProjectFileList(
        authStore.email,
                    ...
                    onClick={async () => {
                      try {
                        if (!authStore.email || !projectStore.activeProjectId)
                          return;
                        const folderPath =
                          await getPlatformService().system.getProjectFolderPath(
                            authStore.email,
                            projectStore.activeProjectId
                          );
                        const result = await getPlatformService().system.openInIDE(
                          folderPath,
            ...
            <div className="flex h-[30px] items-center justify-between gap-2">
              <div
                onClick={() => {
                  ...
                  getPlatformService().file.revealInFolder(selectedFile.path);
                }}
  ...
  useEffect(() => {
    const processHtml = async () => {
      ...
      const processedImages = await Promise.all(
        matches.map(async (match) => {
          ...
          try {
            ...
            const dataUrl =
              await getPlatformService().file.readFileAsDataUrl(imagePath);
      ...
      for (const cssFile of cssFiles) {
        try {
          const cssContent = await getPlatformService().file.openFile(
            'css',
            cssFile.path,
            false
          );
      ...
      for (const jsFile of jsFiles) {
        try {
          const jsContent = await getPlatformService().file.openFile(
            'js',
            jsFile.path,
            false
          );
