...
export default function ChatBox(): JSX.Element {
  ...
  const handleSendShare = useCallback(
    async (token: string) => {
      ...
      if (!hasModel) {
        ...
        navigate('/history?tab=settings&settingsTab=models');
        return;
      }
      if (!privacy) {
      ...
      if (requiresHumanReply) {
        ...
        if (shouldContinueConversation) {
          ...
          if (
            ...
            // Resolve the user's current model config so the backend can
            // switch model mid-conversation when the user changes settings.
            let modelOverride: Record<string, unknown> = {};
            try {
              const { resolveCurrentModelConfig } = await import(
                '@/store/chatStore'
              );
              const resolved = await resolveCurrentModelConfig();
              if (resolved) {
                modelOverride = {
                  model_platform: resolved.model_platform,
              ...
              console.warn(
                '[Multi-turn] Failed to resolve model config, using backend default:',
                err
              );
            }
  ...
  const handleFileSelect = async () => {
    try {
      const result = await getPlatformService().file.selectFile({
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <span className="flex-1 cursor-pointer text-label-xs font-medium leading-normal text-text-information">
                      {t('layout.by-messaging-hanggent')}{' '}
                      <a
                        href="https://www.hangent.com/terms-of-use"
                        target="_blank"
                      ...
                      {t('layout.and')}{' '}
                      <a
                        href="https://www.hangent.com/privacy-policy"
                        target="_blank"
                        className="text-text-information underline"
                        onClick={(e) => e.stopPropagation()}
                        rel="noreferrer"
