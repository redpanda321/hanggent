...
function HeaderWin() {
  ...
  const exportLog = async () => {
    try {
      const response = await getPlatformService().log.exportLog();
      ...
      if (response.savedPath) {
        window.location.href =
          'https://github.com/Hanggent/hanggent/issues/new/choose';
  ...
  const getReferFriendsLink = async () => {
    try {
      ...
      if (res?.invite_code) {
        const inviteLink = `https://www.hangent.com/signup?invite_code=${res.invite_code}`;
  ...
  return (
    <div
      className={`drag absolute left-0 right-0 top-0 z-50 flex !h-9 items-center justify-between py-1 ${platform === 'darwin' ? 'pl-20' : 'pl-2'
        }`}
      id="titlebar"
      ref={titlebarRef}
    >
        ...
        <div className="relative z-50 flex h-full items-center">
          ...
          {location.pathname !== '/history' && (
            <div className="mr-1 flex items-center">
              ...
              {features.gateway && (
              <TooltipSimple
                content={t('layout.gateway', 'Gateway')}
                side="bottom"
                align="center"
              >
                <Button
              ...
              >
                <Button
                  ...
                  size="icon"
                  className="no-drag"
                  onClick={() => window.open('https://api.hangent.com', '_blank')}
                >
                  <Cable className="h-4 w-4" />
                </Button>
              </TooltipSimple>
              <TooltipSimple
                content="Hanggent Video"
                side="bottom"
                align="center"
              >
                <Button
                  ...
                  size="icon"
                  className="no-drag"
                  onClick={() => window.open('https://video.hangent.com', '_blank')}
                >
      ...
      {/* Custom window controls only for Linux (Windows, macOS, and web use native/no controls) */}
      {platform !== 'darwin' && platform !== 'win32' && platform !== 'web' && (
        <div
        ...
        >
          ...
          <div
            className="flex h-full w-[35px] flex-1 cursor-pointer items-center justify-center text-center leading-5 hover:bg-surface-hover-subtle"
            onClick={() => getPlatformService().window.closeWindow()}
          >
            <X className="h-4 w-4" />
          </div>
        </div>
