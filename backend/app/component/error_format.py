...
def normalize_error_to_openai_format(
    ...
    if error_obj is None:
        ...
        if (
            ...
            or "clerk" in lower
            and ("502" in lower or "authenticate" in lower)
        ):
            error_code = "gateway_auth_error"
            message = (
                "The model API endpoint returned an authentication error "
                "(possibly a misconfigured proxy or Clerk gateway). "
                "Please verify the endpoint URL and API key."
            )
            error_obj = {
                "message": message,
