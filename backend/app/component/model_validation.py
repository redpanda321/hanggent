...
def create_agent(
    ...
    **kwargs,
    ...
    # Gemini's underlying REST API expects model names in the form
    # `models/<model-id>`. Some gateways don't auto-prefix this when using the
    # OpenAI-compatible endpoint, leading to INVALID_ARGUMENT.
    if str(platform).lower() == "gemini":
        mtype_str = str(mtype).strip()
        if mtype_str and not mtype_str.startswith("models/"):
            mtype = f"models/{mtype_str}"
