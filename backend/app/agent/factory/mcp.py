...
async def mcp_agent(options: Chat):
    ...
    return ListenChatAgent(
        ...
        model=ModelFactory.create(
            ...
            model_type=(
                f"models/{str(options.model_type).strip()}"
                if str(options.model_platform).lower() == "gemini"
                and options.model_type
                and not str(options.model_type).strip().startswith("models/")
                else options.model_type
            ),
