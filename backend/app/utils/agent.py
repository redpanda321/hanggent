...
from app.utils.toolkit.job_analyzer_toolkit import JobAnalyzerToolkit
from app.utils.toolkit.resume_tailor_toolkit import ResumeTailorToolkit
...
from app.service.usage_service import record_agent_usage
...
class ListenChatAgent(ChatAgent):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.project_id = project_id or api_task_id
        self._step_count = 0
    ...
    def step(
        ...
        try:
            ...
            total_tokens = 0
            # Track failed usage
            self._record_usage_async(0, 0, 0, step_start_time, False, str(e))
        ...
        if res is not None:
            if isinstance(res, StreamingChatAgentResponse):
                # Capture self references for closure
                agent_self = self
                    ...
                    try:
                        ...
                        asyncio.create_task(
                            task_lock.put_queue(
                                ActionDeactivateAgentData(
                                    data={
                                        "agent_name": agent_self.agent_name,
                                        "process_task_id": agent_self.process_task_id,
                        ...
                        agent_self._record_usage_async(input_tokens, output_tokens, total_tokens, step_start_time, True, None)
            ...
            # Track successful usage
            self._record_usage_async(input_tokens, output_tokens, total_tokens, step_start_time, True, None)
    ...
    def _record_usage_async(
        ...
        if not self.model_platform or not self.model_type:
            return
...
def agent_model(
    ...
    toolkits_to_register_agent: list[RegisteredAgentToolkit] | None = None,
    enable_snapshot_clean: bool = False,
    complexity: str | None = None,  # Optional complexity override: "simple", "moderate", "complex"
):
    from app.model.model_routing import TaskComplexity, get_default_complexity_for_agent
    ...
    # Get agent-specific model configuration (supports multi-model routing)
    task_complexity = None
    if complexity:
        task_complexity = TaskComplexity(complexity)
    else:
        task_complexity = get_default_complexity_for_agent(agent_name)
    
    model_config_obj = options.get_model_config(agent_name, task_complexity)
    
    # Use model config from routing, fallback to Chat fields for api_key/url if not set
    model_platform = model_config_obj.model_platform
    model_type = model_config_obj.model_type
    api_key = model_config_obj.api_key or options.api_key
    api_url = model_config_obj.api_url or options.api_url
    extra_params = model_config_obj.extra_params or options.extra_params or {}
    ...
    if agent_name == Agents.browser_agent:
        try:
            model_platform_enum = ModelPlatformType(model_platform.lower())
            ...
            traceroot_logger.error(
                f"Invalid model platform for browser agent: {model_platform}",
                exc_info=True,
            )
            model_platform_enum = None
    
    effective_model_type = model_type
    if str(model_platform).lower() == "gemini":
        mtype_str = str(model_type or "").strip()
        if mtype_str and not mtype_str.startswith("models/"):
            effective_model_type = f"models/{mtype_str}"

    traceroot_logger.info(
        f"Agent {agent_name} using model: {model_platform}/{effective_model_type} (complexity: {task_complexity})"
    )
...
async def create_agent_with_fallback(
    ...
    """
    Create an agent with auto-scaling fallback support.
    
    If the primary model fails and auto-scaling is enabled:
    1. Retry with exponential backoff up to max_retries
    2. If still failing, switch to fallback model
    
    This function wraps agent_model with retry and fallback logic.
    """
    from app.model.model_routing import AutoScalingConfig
    
    # Get auto-scaling config from options
    auto_scaling: AutoScalingConfig | None = None
    if options.model_routing and hasattr(options.model_routing, 'auto_scaling'):
        auto_scaling = options.model_routing.auto_scaling
    
    # If no auto-scaling or fallback disabled, just create agent normally
    if not auto_scaling or not auto_scaling.fallback_enabled:
        return agent_model(
            agent_name=agent_name,
            system_message=system_message,
            options=options,
    ...
    max_retries = auto_scaling.max_retries
    retry_delay = auto_scaling.retry_delay_seconds
    last_error = None
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            traceroot_logger.warning(
                f"Agent {agent_name} creation failed (attempt {attempt + 1}/{max_retries + 1}): {e}"
            )
            if attempt < max_retries:
                await asyncio.sleep(retry_delay * (2 ** attempt))  # Exponential backoff
    
    # All retries failed, try fallback model if configured
    if auto_scaling.fallback_model:
        traceroot_logger.info(
            f"Switching to fallback model for agent {agent_name}: "
            f"{auto_scaling.fallback_model.model_platform}/{auto_scaling.fallback_model.model_type}"
        )
        
        # Create a modified options with fallback model as default
        from copy import deepcopy
        from app.model.model_routing import ModelRoutingConfig
        
        fallback_options = deepcopy(options)
        if fallback_options.model_routing:
            # Override the default model with fallback
            fallback_options.model_routing.default = auto_scaling.fallback_model
            # Clear agent overrides for this agent so it uses the fallback
            if agent_name in fallback_options.model_routing.agent_overrides:
                del fallback_options.model_routing.agent_overrides[agent_name]
        else:
            # Create a new routing config with just the fallback model
            fallback_options.model_routing = ModelRoutingConfig(
                default=auto_scaling.fallback_model
            )
        ...
        try:
            ...
            traceroot_logger.error(
                f"Fallback model also failed for agent {agent_name}: {fallback_error}"
            )
            raise fallback_error
    
    # No fallback configured, raise the last error
    if last_error:
        raise last_error
    raise RuntimeError(f"Failed to create agent {agent_name} after all retries")


async def check_cost_limit_and_downgrade(
    options: Chat,
    agent_name: str,
) -> Chat:
    """
    Check current usage against cost limits and optionally downgrade to cheaper model.
    
    Returns potentially modified options with downgraded model if cost limit threshold reached.
    """
    from app.model.model_routing import AutoScalingConfig
    ...
    if not auto_scaling or not auto_scaling.cost_limit_enabled:
        return options
    
    # Check if we have cost limits set
    daily_limit = auto_scaling.daily_cost_limit
    monthly_limit = auto_scaling.monthly_cost_limit
    ...
    try:
        ...
        base_url = os.environ.get("SERVER_URL", "http://localhost:5173")
        auth_token = getattr(options, 'auth_token', None) or os.environ.get("AUTH_TOKEN", "")
        ...
        async with httpx.AsyncClient() as client:
            ...
            if daily_resp.status_code == 200:
                ...
                if data.get("code") == 200:
                    daily_usage = data.get("data", {}).get("total_cost", 0.0)
            ...
            if monthly_resp.status_code == 200:
                data = monthly_resp.json()
                if data.get("code") == 200:
                    monthly_usage = data.get("data", {}).get("total_cost", 0.0)
        
        # Check thresholds
        downgrade_threshold = auto_scaling.downgrade_at_percentage / 100.0
        should_downgrade = False
        
        if daily_limit and daily_usage >= (daily_limit * downgrade_threshold):
            traceroot_logger.warning(
                f"Daily cost limit {downgrade_threshold*100}% reached: ${daily_usage:.4f} / ${daily_limit:.2f}"
            )
            should_downgrade = True
        
        if monthly_limit and monthly_usage >= (monthly_limit * downgrade_threshold):
            traceroot_logger.warning(
                f"Monthly cost limit {downgrade_threshold*100}% reached: ${monthly_usage:.4f} / ${monthly_limit:.2f}"
            )
            should_downgrade = True
        
        if should_downgrade and auto_scaling.cost_limit_fallback_model:
            traceroot_logger.info(
                f"Auto-downgrading agent {agent_name} to cost-limit fallback model: "
                f"{auto_scaling.cost_limit_fallback_model.model_platform}/"
                f"{auto_scaling.cost_limit_fallback_model.model_type}"
            )
            
            from copy import deepcopy
            from app.model.model_routing import ModelRoutingConfig
            
            downgraded_options = deepcopy(options)
            if downgraded_options.model_routing:
                downgraded_options.model_routing.default = auto_scaling.cost_limit_fallback_model
                if agent_name in downgraded_options.model_routing.agent_overrides:
                    del downgraded_options.model_routing.agent_overrides[agent_name]
            else:
                downgraded_options.model_routing = ModelRoutingConfig(
                    default=auto_scaling.cost_limit_fallback_model
                )
            return downgraded_options
        
    except Exception as e:
        traceroot_logger.error(f"Error checking cost limits: {e}")
...
# =============================================================================
# Job Hunt Workforce Agents
# =============================================================================

@traceroot.trace()
async def job_scraper_agent(options: Chat):
    """
    Agent specialized in scraping job listings from multiple job boards.
    
    Uses JobSpy to search LinkedIn, Indeed, Glassdoor, ZipRecruiter, and Google Jobs.
    Default: LinkedIn and Indeed, last 7 days, with URL-based deduplication.
    """
    working_directory = get_working_directory(options)
    traceroot_logger.info(
        f"Creating job scraper agent for project: {options.project_id}"
    )
    
    message_integration = ToolkitMessageIntegration(
        message_handler=HumanToolkit(
            options.project_id, Agents.job_scraper_agent
        ).send_message_to_user
    )
    
    job_scraper_toolkit = JobScraperToolkit(
        api_task_id=options.project_id,
        working_directory=working_directory,
    )
    job_scraper_toolkit = message_integration.register_toolkits(job_scraper_toolkit)
    
    note_toolkit = NoteTakingToolkit(
        api_task_id=options.project_id,
        agent_name=Agents.job_scraper_agent,
        working_directory=working_directory,
    )
    note_toolkit = message_integration.register_toolkits(note_toolkit)
    
    tools = [
        *job_scraper_toolkit.get_tools(),
        *HumanToolkit.get_can_use_tools(options.project_id, Agents.job_scraper_agent),
        *note_toolkit.get_tools(),
    ]
    
    system_message = f"""
<role>
You are a Job Scraper Agent specialized in finding job opportunities from multiple job boards.
Your primary tool is python-jobspy which can scrape LinkedIn, Indeed, Glassdoor, ZipRecruiter, and Google Jobs.
</role>
...
<capabilities>
- Search multiple job boards simultaneously (default: LinkedIn, Indeed)
- Filter by location, salary, remote options, job type
- Extract detailed job descriptions
- Track newly posted jobs (default: last 7 days)
- Deduplicate jobs across sessions using URL hashing
</capabilities>

<workflow>
1. Receive search criteria (job title, location, preferences)
2. Execute job searches across specified platforms
3. Deduplicate results against previously scraped jobs
4. Store results and save to working directory
5. Record findings in notes for other agents to access
6. Report summary of new jobs found
</workflow>

<mandatory_instructions>
- Always use the `write_note` tool to record job search results for other agents
- Report the number of new jobs found vs duplicates filtered
- Save job listings to JSON and CSV in the working directory
- Include job URLs for easy access
</mandatory_instructions>
"""
    
    return agent_model(
        Agents.job_scraper_agent,
        BaseMessage.make_assistant_message(role_name="Job Scraper Agent", content=system_message),
        options,
        tools,
        tool_names=[
            JobScraperToolkit.toolkit_name(),
            HumanToolkit.toolkit_name(),
            NoteTakingToolkit.toolkit_name(),
        ],
    )
...
@traceroot.trace()
async def job_analyzer_agent(options: Chat):
    """
    Agent specialized in analyzing job-resume fit and generating match scores.
    
    Parses resumes, extracts job requirements, calculates match scores,
    and generates detailed analysis reports.
    """
    working_directory = get_working_directory(options)
    traceroot_logger.info(
        f"Creating job analyzer agent for project: {options.project_id}"
    )
    
    message_integration = ToolkitMessageIntegration(
        message_handler=HumanToolkit(
            options.project_id, Agents.job_analyzer_agent
        ).send_message_to_user
    )
    
    analyzer_toolkit = JobAnalyzerToolkit(
        api_task_id=options.project_id,
        working_directory=working_directory,
    )
    analyzer_toolkit = message_integration.register_toolkits(analyzer_toolkit)
    
    note_toolkit = NoteTakingToolkit(
        api_task_id=options.project_id,
        agent_name=Agents.job_analyzer_agent,
        working_directory=working_directory,
    )
    note_toolkit = message_integration.register_toolkits(note_toolkit)
    
    markitdown_toolkit = MarkItDownToolkit(api_task_id=options.project_id)
    markitdown_toolkit = message_integration.register_toolkits(markitdown_toolkit)
    
    tools = [
        *analyzer_toolkit.get_tools(),
        *HumanToolkit.get_can_use_tools(options.project_id, Agents.job_analyzer_agent),
        *note_toolkit.get_tools(),
    ...
    system_message = f"""
<role>
You are a Job Analyzer Agent - an expert recruiter and career advisor.
You analyze job descriptions against resumes to determine compatibility and provide actionable insights.
</role>
...
- Calculate match scores with detailed breakdown:
  * Skill Match (40% weight): Technical and soft skills alignment
  * Experience Match (30% weight): Years and relevance of experience
  * Education Match (15% weight): Degree and field alignment
  * Keyword Overlap (15% weight): ATS-relevant keyword matching
- Generate comprehensive analysis reports
- Identify skill gaps and provide improvement recommendations
</capabilities>

<scoring_levels>
- Excellent (80-100): Strong match, high interview likelihood
- Good (65-79): Competitive candidate, worth applying
- Fair (50-64): Potential fit, address skill gaps
- Poor (<50): Consider alternative roles
</scoring_levels>

<workflow>
1. Read notes from Job Scraper Agent to get job listings
2. Parse the user's resume to extract skills, experience, education
3. For each job, extract requirements and calculate match score
4. Rank jobs by match score
5. Generate analysis reports for top matches
6. Record analysis results in notes for Resume Tailor Agent
</workflow>

<mandatory_instructions>
- Always read notes from Job Scraper Agent first
- Use the `write_note` tool to record analysis results
- Be honest about skill gaps - never overstate matches
- Provide specific, actionable recommendations
</mandatory_instructions>
"""
    
    return agent_model(
        Agents.job_analyzer_agent,
        BaseMessage.make_assistant_message(role_name="Job Analyzer Agent", content=system_message),
        options,
        tools,
        tool_names=[
            JobAnalyzerToolkit.toolkit_name(),
            HumanToolkit.toolkit_name(),
            NoteTakingToolkit.toolkit_name(),
            MarkItDownToolkit.toolkit_name(),
        ],
...
@traceroot.trace()
async def resume_tailor_agent(options: Chat):
    """
    Agent specialized in creating job-tailored resumes.
    
    Uses AIHawk methodology to optimize resumes for specific job postings
    while maintaining truthfulness and ATS compatibility.
    """
    working_directory = get_working_directory(options)
    traceroot_logger.info(
        f"Creating resume tailor agent for project: {options.project_id}"
    )
    
    message_integration = ToolkitMessageIntegration(
        message_handler=HumanToolkit(
            options.project_id, Agents.resume_tailor_agent
        ).send_message_to_user
    )
    
    tailor_toolkit = ResumeTailorToolkit(
        api_task_id=options.project_id,
        chat_options=options,
        working_directory=working_directory,
    )
    tailor_toolkit = message_integration.register_toolkits(tailor_toolkit)
    
    note_toolkit = NoteTakingToolkit(
        api_task_id=options.project_id,
        agent_name=Agents.resume_tailor_agent,
        working_directory=working_directory,
    )
    note_toolkit = message_integration.register_toolkits(note_toolkit)
    ...
    tools = [
        *tailor_toolkit.get_tools(),
        *HumanToolkit.get_can_use_tools(options.project_id, Agents.resume_tailor_agent),
        *note_toolkit.get_tools(),
    ...
    system_message = f"""
<role>
You are a Resume Tailor Agent - an expert resume strategist.
You create job-specific tailored resumes that maximize match scores while remaining completely truthful.
Your methodology is based on AIHawk (Jobs_Applier_AI_Agent).
</role>
...
<core_principles>
1. **PRESERVE**: Maintain original formatting, structure, and layout
2. **REWRITE**: Adjust content to highlight alignment with job description
3. **NEVER FABRICATE**: Never add skills or experiences not in the original resume
4. **OPTIMIZE FOR ATS**: Use job-specific keywords naturally
5. **MAINTAIN PROFESSIONALISM**: Keep language concise and impactful
</core_principles>

<capabilities>
- Tailor resume content to match specific job requirements
- Optimize for Applicant Tracking Systems (ATS)
- Generate tailored cover letters
- Export to multiple formats: PDF, DOCX, Markdown
- Track optimizations made for transparency
</capabilities>

<workflow>
1. Read notes from Job Analyzer Agent to get top-matching jobs
2. For each high-scoring job (typically top 3):
   a. Get the job description and matching/missing skills
   b. Tailor resume content to emphasize relevant experience
   c. Optimize for ATS with appropriate keywords
   d. Generate a tailored cover letter
   e. Export to PDF and DOCX formats
3. Record all tailored resumes in notes with file paths
4. Provide summary of all created documents
</workflow>

<mandatory_instructions>
- Always read notes from Job Analyzer Agent first
- Only tailor resumes for jobs with score >= 50
- Create both PDF and DOCX versions
- Use the `write_note` tool to record file paths
...
"""
    ...
    return agent_model(
        ...
        BaseMessage.make_assistant_message(role_name="Resume Tailor Agent", content=system_message),
        options,
        tools,
        tool_names=[
            ResumeTailorToolkit.toolkit_name(),
            HumanToolkit.toolkit_name(),
            NoteTakingToolkit.toolkit_name(),
            FileToolkit.toolkit_name(),
        ],
