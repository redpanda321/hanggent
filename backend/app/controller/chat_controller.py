...
def normalize_runtime_api_url(
    model_platform: str, api_url: str | None
) -> str | None:
    if api_url is None:
        return None

    if str(model_platform).strip().lower() != "anthropic":
        return api_url

    stripped = str(api_url).strip().rstrip("/")
    if stripped.lower() == "https://api.anthropic.com":
...
async def timeout_stream_wrapper(
    ...
    """Wraps a stream generator with timeout handling and SSE keepalive.

    Closes the SSE connection if no data is received within the timeout period.
    Sends SSE comment keepalives every heartbeat_interval seconds to prevent
    HTTP/2 idle stream resets at the proxy layer.
        ...
        while True:
            ...
            try:
                data = await asyncio.wait_for(
                    generator.__anext__(),
                    timeout=min(heartbeat_interval, remaining_timeout),
                )
                last_data_time = time.time()
                yield data
            except TimeoutError:
                # Heartbeat interval elapsed without data — check if real
                ...
                if time.time() - last_data_time >= timeout_seconds:
                    ...
                    break
                # Not a real timeout — send SSE keepalive comment to prevent
                # HTTP/2 idle stream resets at the proxy layer
                yield ": keepalive\n\n"
...
async def post(data: Chat, request: Request):
    ...
    normalized_api_url = normalize_runtime_api_url(
        data.model_platform, data.api_url
    )
    # Propagate the normalized URL back so agent_model() receives the
    # corrected value (e.g. Anthropic "…/v1" suffix) instead of the raw one.
    if normalized_api_url is not None:
        data.api_url = normalized_api_url
    os.environ["OPENAI_API_BASE_URL"] = (
        normalized_api_url or "https://api.openai.com/v1"
    )
    ...
    if data.task_id:
        try:
            ...
            if current_file_save_path:
                ...
                if len(path_parts) >= 3 and "hanggent" in path_parts:
                    hanggent_index = path_parts.index("hanggent")
                    if hanggent_index + 1 < len(path_parts):
                        current_email = path_parts[hanggent_index + 1]
