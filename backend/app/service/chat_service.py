        ...
        try:
            if item.action == Action.improve or start_event_loop:
                ...
                if start_event_loop is True:
                    ...
                    # Check if the user switched model mid-conversation
                    if item.data.has_model_override():
                        old_platform = options.model_platform
                        old_type = options.model_type
                        options.model_platform = item.data.model_platform
                        options.model_type = item.data.model_type
                        if item.data.api_key is not None:
                            options.api_key = item.data.api_key
                        if item.data.api_url is not None:
                            options.api_url = item.data.api_url
                        if item.data.extra_params is not None:
                            options.extra_params = item.data.extra_params
                        logger.info(
                            f"[MODEL-SWITCH] User switched model "
                            f"from {old_platform}/{old_type} to "
                            f"{options.model_platform}/{options.model_type}"
                        )
                        # Recreate question_agent with the new model config
                        if task_lock.question_agent is not None:
                            task_lock.question_agent.reset()
                        task_lock.question_agent = (
                            question_confirm_agent(options)
                        )
                        question_agent = task_lock.question_agent
                        logger.info(
                            "[MODEL-SWITCH] Recreated question_agent "
                            "with new model config"
                        )
                    ...
                    async def run_decomposition():
                        ...
                        try:
                            sub_tasks = await asyncio.to_thread(
                                workforce.hanggent_make_sub_tasks,
                                camel_task,
                ...
                # workforce.hanggent_start uses correct list
                ...
                if not sub_tasks:
                    sub_tasks = getattr(task_lock, "decompose_sub_tasks", [])
                task = asyncio.create_task(workforce.hanggent_start(sub_tasks))
                ...
                if camel_task is None:
                    ...
                    if workforce is not None:
                        task = asyncio.create_task(
                            workforce.hanggent_start(camel_task.subtasks)
                        )
                        task_lock.add_background_task(task)
...
the current date.
    ...
    try:
        ...
        results = await asyncio.gather(
            ...
            mcp_agent(options),
            opencode_agent(options),
            openclaw_agent(options),
        )
    except Exception as e:
        logger.error(
    ...
    workforce.add_single_agent_worker(
        "OpenCode Agent: An AI coding agent that can read, search, edit, "
        "and write code across entire codebases. Specializes in software "
        "development tasks including implementing features, debugging, "
        "refactoring, writing tests, and code review. Has native file "
        "editing, code search, shell execution, LSP integration, and git "
        "support. Prefer this agent for complex codebase-wide coding tasks.",
        opencode,
    )
    workforce.add_single_agent_worker(
        "OpenClaw Agent: A multi-channel messaging gateway specialist "
        "that can send and receive messages across WhatsApp, Telegram, "
        "Slack, Discord, Signal, iMessage, Google Chat, and other "
        "platforms. Manages channel connections, session orchestration, "
        "automated cron scheduling, and AI-powered conversational flows. "
        "Prefer this agent for cross-platform messaging, notifications, "
        "and communication automation tasks.",
        openclaw,
    )
