...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  services: {
    ...
    restart: (name: string) => ipcRenderer.invoke('services:restart', name),
    checkHealth: (name: string) =>
      ipcRenderer.invoke('services:checkHealth', name),
    getLogs: (source?: string) =>
      ipcRenderer.invoke('services:getLogs', source),
    clearLogs: (source: string) =>
      ipcRenderer.invoke('services:clearLogs', source),
    updateConfig: (name: string, updates: any) =>
      ipcRenderer.invoke('services:updateConfig', name, updates),
    onStatusChange: (callback: (state: any) => void) => {
      const listener = (_event: any, state: any) => callback(state);
      ipcRenderer.on('service:status', listener);
      return () => {
        ipcRenderer.off('service:status', listener);
      };
    },
    onLog: (callback: (entry: any) => void) => {
      ...
      return () => {
        ipcRenderer.off('service:log', listener);
      };
    },
  },
});
