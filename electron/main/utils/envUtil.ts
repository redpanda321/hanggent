...
export function getEnvPath(email: string) {
  const tempEmail = email
    ...
    .replace(/[\\/*?:"<>|\s]/g, '_')
    .replace('.', '_');
  const hanggentDir = path.join(os.homedir(), '.hanggent');

  // Ensure .hanggent directory exists
  if (!fs.existsSync(hanggentDir)) {
    fs.mkdirSync(hanggentDir, { recursive: true });
  }

  const envPath = path.join(hanggentDir, '.env.' + tempEmail);
...
/**
 * Read the value of a key from the global ~/.hanggent/.env file.
 */
export function readGlobalEnvKey(key: string): string | null {
  try {
    const globalEnvPath = path.join(os.homedir(), '.hanggent', '.env');
...
export function getEmailFolderPath(email: string) {
  const tempEmail = email
    ...
    .replace(/[\\/*?:"<>|\s]/g, '_')
    .replace('.', '_');
  const MCP_CONFIG_DIR = path.join(os.homedir(), '.hanggent');
