...
export async function getBinaryPath(name?: string): Promise<string> {
  ...
  const binariesDir = path.join(os.homedir(), '.hanggent', 'bin');

  // Ensure .hanggent/bin directory exists
...
export function getCachePath(folder: string): string {
  ...
  const cacheDir = path.join(os.homedir(), '.hanggent', 'cache', folder);
...
export function getVenvsBaseDir(): string {
  return path.join(os.homedir(), '.hanggent', 'venvs');
}
