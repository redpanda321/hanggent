...
export class FileReader {
  ...
  public getFileList(
    ...
    if (projectId) {
      ...
      const userDir = path.join(userHome, 'hanggent', safeEmail);
      ...
      if (projectBasedPath) {
        ...
        dirPath = path.join(userHome, 'hanggent', safeEmail, `task_${taskId}`);
      }
    }
    ...
    if (projectId) {
      ...
      if (projectBasedPath) {
        ...
        if (projectMatch) {
          ...
          logPath = path.join(userHome, '.hanggent', safeEmail, `task_${taskId}`);
        }
      } else {
        // Fallback to legacy direct task structure
        dirPath = path.join(userHome, 'hanggent', safeEmail, `task_${taskId}`);
  ...
  public getLogFolder(email: string): string {
    const safeEmail = email
      ...
      .replace(/^\.+|\.+$/g, '');
    const userHome = app.getPath('home');
    const dirPath = path.join(userHome, 'hanggent', safeEmail);
