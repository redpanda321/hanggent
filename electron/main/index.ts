...
// Storage strategy:
// 1. Main window: partition 'persist:main_window' in app userData → Hanggent account (persistent)
// 2. WebView: partition 'persist:user_login' in app userData → will import cookies from tool_controller via session API
// 3. tool_controller: ~/.hanggent/browser_profiles/profile_user_login → source of truth for login cookies
...
// Override User Agent to remove Electron/hanggent identifiers
...
const setupProtocolHandlers = () => {
  if (process.env.NODE_ENV === 'development') {
    const isDefault = app.isDefaultProtocolClient('hanggent', process.execPath, [
      path.resolve(process.argv[1]),
    ]);
    if (!isDefault) {
      app.setAsDefaultProtocolClient('hanggent', process.execPath, [
        path.resolve(process.argv[1]),
      ]);
    }
  } else {
    app.setAsDefaultProtocolClient('hanggent');
  }
};
...
const setupSingleInstanceLock = () => {
  ...
  if (!gotLock) {
    ...
    app.on('second-instance', (event, argv) => {
      ...
      const url = argv.find((arg) => arg.startsWith('hanggent://'));
...
function registerIpcHandlers() {
  ...
  ipcMain.handle(
    ...
    async (event, command: string, email: string) => {
      ...
      try {
        ...
        // Add --host parameter
        const commandWithHost = `${command} --debug --host dev.hanggent.ai/api/oauth/notion/callback?code=1`;
  ...
  ipcMain.handle('export-log', async () => {
    try {
      ...
      const defaultFileName = `hanggent-${appVersion}-${systemVersion}-${Date.now()}.log`;
  ...
  ipcMain.handle('env-write', async (_event, email, { key, value }) => {
    ...
    const GLOBAL_ENV_PATH = path.join(os.homedir(), '.hanggent', '.env');
    let globalContent = '';
    try {
...
// ==================== ensure hanggent directories ====================
const ensureHanggentDirectories = () => {
  const hanggentBase = path.join(os.homedir(), '.hanggent');
  const legacyBase = path.join(os.homedir(), '.eigent');

  // Migrate from legacy ~/.eigent/ to ~/.hanggent/ if needed
  if (fs.existsSync(legacyBase) && !fs.existsSync(hanggentBase)) {
    log.info(`Migrating legacy directory: ${legacyBase} -> ${hanggentBase}`);
    try {
      fs.renameSync(legacyBase, hanggentBase);
      log.info('Legacy .eigent directory migrated to .hanggent');
    } catch (err) {
      log.error('Failed to migrate .eigent directory, will create fresh:', err);
    }
  }

  const requiredDirs = [
    hanggentBase,
    path.join(hanggentBase, 'bin'),
    path.join(hanggentBase, 'cache'),
    path.join(hanggentBase, 'venvs'),
    path.join(hanggentBase, 'runtime'),
  ];
  ...
  log.info('.hanggent directory structure ensured');
};
...
async function createWindow() {
  ...
  // Ensure .hanggent directories exist before anything else
  ensureHanggentDirectories();
  ...
  if (win && !win.isDestroyed()) {
    ...
    // Set main window reference for service status forwarding
    try {
      const serviceManager = getServiceManager();
      if (serviceManager) {
        serviceManager.setMainWindow(win);
        log.info('[SERVICES] Main window set for service status forwarding');
      }
    } catch (error) {
      log.warn('[SERVICES] Could not set main window for services:', error);
    }
  }
...
app.whenReady().then(async () => {
  ...
  // ==================== initialize external services (OpenCode, OpenClaw) ====================
  try {
    await initializeServices();
    log.info('[SERVICES] External services initialized successfully');
  } catch (error) {
    log.error('[SERVICES] Failed to initialize external services:', error);
    // Non-fatal: app continues without external services
  }
...
app.on('before-quit', async (event) => {
  ...
  try {
    ...
    // Clean up external services (OpenCode, OpenClaw)
    try {
      await cleanupServices();
      log.info('[SERVICES] External services cleaned up');
    } catch (error) {
      log.error('[SERVICES] Error cleaning up external services:', error);
    }
